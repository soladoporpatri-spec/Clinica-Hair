...
public class SubscriptionService
{
    ...
    /// profissional — credito do "Plano com Profissional A" nao e descontado com outro profissional.
    ...
    /// Ex.: "com Profissional A" ou "qualquer profissional".
    /// </summary>
    ...
    public async Task<ClientSubscription?> ActivateAsync(int id, string? confirmedBy = null, string? notes = null, CancellationToken ct = default)
    {
        var sub = GetById(id);
        ...
        sub.PaymentConfirmedAt = DateTime.Now;
        sub.PaymentConfirmedBy = string.IsNullOrWhiteSpace(confirmedBy) ? "Admin" : confirmedBy.Trim();
        if (!string.IsNullOrWhiteSpace(notes))
            sub.Notes = string.IsNullOrWhiteSpace(sub.Notes) ? notes.Trim() : $"{sub.Notes}\n{notes.Trim()}";
        await _db.SaveChangesAsync(ct);
        return sub;
    }
