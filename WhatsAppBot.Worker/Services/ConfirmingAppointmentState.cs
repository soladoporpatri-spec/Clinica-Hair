        ...
        await HandlePollRequiredState(session, text, pollId, ConversationState.AwaitingTimeSelection, "horario", async () =>
        {
            ...
            if (isBarbershop)
            {
                try
                {
                    ...
                    // Regras: credito de "Plano Corte" nao cobre barba; credito "com Profissional A" nao
            ...
            // A confirmação final é deliberadamente fixa. O template legado
            // Msg_Confirmation era uma pergunta de pré-confirmação e, por ser
            // configurável no banco, voltava a aparecer depois do voto, com
            // placeholders sem valor. A enquete anterior já coletou a decisão.
            string confirmMsg;
            if (wasWalkIn)
            {
            ...
            {
                var detailConfirmation = usesDetails
                    ? string.IsNullOrWhiteSpace(detail) ? "" : $"\n{labels.DetailLineLabel}: {detail}"
                    : $"\n{labels.ProfissionalSingular}: {appt.BarberName ?? "nossa equipe"}";
                confirmMsg = $"✅ *Agendamento confirmado!*\n\n" +
                             $"📅 Data: {appt.DateTime:dd/MM/yyyy}\n" +
                             $"⏰ Horário: {appt.DateTime:HH:mm}\n" +
                             $"{labels.ServiceEmoji} Serviço: {sInfo.Name}" +
                             detailConfirmation + subscriptionNote;
            }
