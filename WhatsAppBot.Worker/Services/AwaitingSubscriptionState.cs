...
public class AwaitingSubscriptionState : ConversationStateBase
{
    ...
    private async Task HandleConfirmationAsync(ConversationSession session, string text, string? pollId, CancellationToken ct)
    {
            ...
            ConversationState.AwaitingSubscription, "planos", async () =>
        {
            ...
            var confirmStoreName = confirmStore?.Name ?? "nossa empresa";
            var pixCharge = await _pix.CreateSubscriptionChargeAsync(sub, confirmStore, ct);
            var pixKey = pixCharge.PixKey;
            var pixRecipient = "";
            var pixLine = !pixCharge.IsConfigured
                ? "entre em contato com nossa equipe para enviar o pagamento."
                : $"faça o PIX de *R${plan.Preco:F2}*{pixRecipient} para a chave:\n*{pixKey}*";

            if (pixCharge.IsConfigured)
            {
                pixLine = $"faca o PIX de *R$ {pixCharge.Amount:0.00}* para a chave:\n" +
                          $"*{pixCharge.PixKey}*\n\n" +
                          $"Identificador: *{pixCharge.TxId}*\n\n" +
                          $"PIX copia e cola:\n{pixCharge.Payload}";
            }
