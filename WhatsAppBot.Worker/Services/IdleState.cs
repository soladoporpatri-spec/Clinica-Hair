...
public class IdleState : ConversationStateBase
{
    ...
    public override async Task HandleAsync(ConversationSession session, string text, string? pollId, CancellationToken ct)
    {
        ...
        var welcomeTemplate = ExtractGreetingOnly(Settings.GetString("Msg_Welcome"));
    ...
    /// <summary>
    /// Compatibilidade com configuracoes antigas que gravavam o menu numerico dentro
    /// de Msg_Welcome. As opcoes pertencem exclusivamente a enquete enviada logo depois.
    /// Preserva somente a saudacao anterior ao marcador do menu.
    /// </summary>
    private static string ExtractGreetingOnly(string? template)
    {
        ...
        {
            var index = template.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
            if (index >= 0)
                return template[..index].Trim();
        }
