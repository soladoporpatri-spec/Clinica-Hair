...
public static class CustomerCrmEndpoints
{
    ...
    public static IEndpointRouteBuilder MapCustomerCrmEndpoints(this IEndpointRouteBuilder app, string apiKey)
    {
        ...
        app.MapDelete("/api/customers/{customerKey}", async (
            HttpContext ctx,
            string customerKey,
            AppDbContext db,
            ITenantService tenantService,
            CancellationToken ct = default) =>
        {
            ...
            var appointments = await db.Appointments
                .Where(a => a.StoreId == guard.StoreId)
                .ToListAsync(ct);
            var customerAppointments = appointments
                .Where(a => CustomerKeyFor(a.PhoneNumber, a.ContactName) == key)
                .ToList();
            var appointmentIds = customerAppointments.Select(a => a.Id).ToHashSet();

            if (appointmentIds.Count > 0)
            {
                var linkedTickets = await db.OptimizationTickets
                    .Where(t => t.StoreId == guard.StoreId && t.AppointmentId.HasValue && appointmentIds.Contains(t.AppointmentId.Value))
                    .ToListAsync(ct);
            ...
            var reminders = await db.CustomerReminders.Where(r => r.StoreId == guard.StoreId && r.CustomerKey == key).ToListAsync(ct);
            var events = await db.CustomerEvents.Where(e => e.StoreId == guard.StoreId && e.CustomerKey == key).ToListAsync(ct);
            var tagAssignments = await db.CustomerTagAssignments.Where(a => a.StoreId == guard.StoreId && a.CustomerKey == key).ToListAsync(ct);
            var profiles = await db.CustomerProfiles.Where(p => p.StoreId == guard.StoreId && p.CustomerKey == key).ToListAsync(ct);

            db.CustomerReminders.RemoveRange(reminders);
            db.CustomerEvents.RemoveRange(events);
            db.CustomerTagAssignments.RemoveRange(tagAssignments);
            db.CustomerProfiles.RemoveRange(profiles);
            db.Appointments.RemoveRange(customerAppointments);
            ...
            return Results.Ok(new
            {
                deleted = new
                {
                    appointments = customerAppointments.Count,
    ...
    {
        var raw = Uri.UnescapeDataString(value ?? "").Trim();
        if (raw.StartsWith("walkin-", StringComparison.OrdinalIgnoreCase)) return raw.ToLowerInvariant();
