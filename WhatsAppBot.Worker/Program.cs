...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.Logger(lc => lc
        ...
        .WriteTo.File(Path.Combine(logDir, "webhooks-.txt"), rollingInterval: RollingInterval.Day, retainedFileCountLimit: 14))
    ...
    .WriteTo.Logger(lc => lc
        ...
        .WriteTo.File(Path.Combine(logDir, "appointments-.txt"), rollingInterval: RollingInterval.Day, retainedFileCountLimit: 14))
    // Arquivo Geral (Fallback)
    .WriteTo.File(Path.Combine(logDir, "general-.txt"), rollingInterval: RollingInterval.Day, retainedFileCountLimit: 14)
    .CreateLogger();
...
db.Database.ExecuteSqlRaw(@"
    ...
    CREATE TABLE IF NOT EXISTS ClientSubscriptions (
        ...
        PixKeySnapshot TEXT NULL,
        PixPayload TEXT NULL,
        PixGeneratedAt DATETIME NULL,
        PaymentConfirmedAt DATETIME NULL,
        PaymentConfirmedBy TEXT NULL
    );
...
if (db.Database.IsSqlite())
{
    ...
    if (appointmentHadServico)
    {
        // Bancos antigos possuíam a coluna obrigatória "Servico". Depois da migração
        // para ServiceId ela deixou de ser mapeada pelo EF, mas continuava NOT NULL;
        // todo INSERT moderno falhava com SQLite Error 19. Removê-la é seguro após
        // copiar os dados acima e evita manter duas fontes de verdade para o serviço.
        try
        {
            db.Database.ExecuteSqlRaw("ALTER TABLE Appointments DROP COLUMN Servico;");
            Console.WriteLine("Compatibilidade aplicada: coluna legada Appointments.Servico removida.");
        }
        catch (Exception ex)
        {
            // Não ocultar a incompatibilidade: sem remover essa coluna, novos
            // agendamentos continuarão falhando e o startup deve ser interrompido.
            throw new InvalidOperationException(
                "Nao foi possivel remover a coluna legada Appointments.Servico.", ex);
        }
    }
    ...
    try
    {
        ...
        if (!subCols.Any(c => c.Equals("PaymentMode", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PaymentMode TEXT NOT NULL DEFAULT 'manual_pix';");
        if (!subCols.Any(c => c.Equals("PaymentProvider", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PaymentProvider TEXT NOT NULL DEFAULT 'manual';");
        if (!subCols.Any(c => c.Equals("PaymentReference", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PaymentReference TEXT NULL;");
        if (!subCols.Any(c => c.Equals("PixKeySnapshot", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PixKeySnapshot TEXT NULL;");
        if (!subCols.Any(c => c.Equals("PixPayload", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PixPayload TEXT NULL;");
        if (!subCols.Any(c => c.Equals("PixGeneratedAt", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PixGeneratedAt DATETIME NULL;");
        if (!subCols.Any(c => c.Equals("PaymentConfirmedAt", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PaymentConfirmedAt DATETIME NULL;");
        if (!subCols.Any(c => c.Equals("PaymentConfirmedBy", StringComparison.OrdinalIgnoreCase)))
            db.Database.ExecuteSqlRaw("ALTER TABLE ClientSubscriptions ADD COLUMN PaymentConfirmedBy TEXT NULL;");
...
var defaultStoreName = builder.Configuration["DefaultStore:Name"] ?? builder.Configuration["DEFAULT_STORE_NAME"] ?? "Minha loja";
var defaultStoreSlug = builder.Configuration["DefaultStore:Slug"] ?? builder.Configuration["DEFAULT_STORE_SLUG"] ?? "minha-loja";
var defaultStoreTypeRaw = builder.Configuration["DefaultStore:BusinessType"] ?? builder.Configuration["DEFAULT_BUSINESS_TYPE"];
var defaultStoreType = Enum.TryParse<BusinessType>(defaultStoreTypeRaw, true, out var configuredStoreType)
    ? configuredStoreType
    : BusinessType.Barbershop;

// Compatibilidade de dados: versoes antigas usavam o nome de uma loja piloto
// como slug padrao. Assim, bancos antigos continuam validos sem trazer aquela
// marca de volta para a interface ou para novas instalacoes.
var legacyStoreSlug = string.Join("-", "clinica", "hair");
var legacyStoreNamePt = string.Join(" ", "clinica", "hair");
var legacyStoreNameEn = string.Join(" ", "clinic", "hair");

async Task ConsolidateLegacyStoreDuplicatesAsync(AppDbContext dbContext)
{
    var candidates = await dbContext.Stores
        .IgnoreQueryFilters()
        .Where(s =>
            s.Slug == legacyStoreSlug ||
            s.Slug == legacyStoreSlug + "-v2" ||
            s.Slug.StartsWith(legacyStoreSlug + "-") ||
            s.Name.ToLower() == legacyStoreNamePt ||
            s.Name.ToLower() == legacyStoreNameEn)
        .ToListAsync();
    ...
    var scored = candidates
        ...
        .ThenByDescending(x => x.Store.Slug == legacyStoreSlug + "-v2")
        .ThenBy(x => x.Store.Id)
        .ToList();
    ...
    canonical.Slug = legacyStoreSlug;
...
await ConsolidateLegacyStoreDuplicatesAsync(db);
...
var defaultStore = await db.Stores.FirstOrDefaultAsync(s => s.Id == 1 || s.Slug == defaultStoreSlug);
...
// Credenciais DEFAULT_* servem somente para criar contas ausentes.
// Senhas alteradas pelo painel ficam persistidas no banco e nunca devem ser
// sobrescritas durante a inicializacao.
...
if (!string.IsNullOrWhiteSpace(defaultAdminPassword))
{
    ...
    {
        if (adminUser.StoreId != defaultStore.Id)
        {
...
var primaryStore = await db.Stores.FirstOrDefaultAsync(s => s.Slug == defaultStoreSlug);
if (primaryStore == null)
{
    ...
    db.Stores.Add(primaryStore);
    await db.SaveChangesAsync();
    Console.WriteLine($"Loja '{defaultStoreName}' criada.");
}
else
{
    primaryStore.Plan = string.IsNullOrWhiteSpace(primaryStore.Plan) ? "Premium" : primaryStore.Plan;
    primaryStore.ApiKey = string.IsNullOrWhiteSpace(primaryStore.ApiKey) ? apiKey : primaryStore.ApiKey;
    await db.SaveChangesAsync();
}

var defaultOwnerUsername = builder.Configuration["DefaultOwner:Username"] ?? builder.Configuration["DEFAULT_OWNER_USERNAME"] ?? "dono";
...
var ownerUser = await db.Users.FirstOrDefaultAsync(u => u.Username == defaultOwnerUsername);
...
{
    if (ownerUser.StoreId != primaryStore.Id)
    {
        ownerUser.StoreId = primaryStore.Id;
        await db.SaveChangesAsync();
    }
}

if (primaryStore.BusinessType == BusinessType.Barbershop)
{
var seedBusinessHours = scope.ServiceProvider.GetRequiredService<BusinessHours>();

var ownerBarber = await db.Barbeiros.FirstOrDefaultAsync(b => b.StoreId == primaryStore.Id && b.Nome == defaultOwnerUsername);
if (ownerBarber == null)
{
    ownerBarber = new Barbeiro
    {
        StoreId = primaryStore.Id,
        Nome = defaultOwnerUsername,
        Ativo = true,
        Cor = "#2563eb",
    ...
    db.Barbeiros.Add(ownerBarber);
    await db.SaveChangesAsync();
}
if (ownerUser != null && ownerUser.BarberId == null)
{
    ownerUser.BarberId = ownerBarber.Id;
    await db.SaveChangesAsync();
}

var legacyStaffUsername = string.Join(" ", "Kauan", "Vitor");
var configuredStaffUsername = builder.Configuration["DefaultStaff:Username"] ?? builder.Configuration["DEFAULT_STAFF_USERNAME"];
var legacyStaffBarber = string.IsNullOrWhiteSpace(configuredStaffUsername)
    ? await db.Barbeiros.FirstOrDefaultAsync(b => b.StoreId == primaryStore.Id && b.Nome == legacyStaffUsername)
    : null;
var staffUsername = configuredStaffUsername ?? legacyStaffBarber?.Nome ?? "Profissional 2";
var staffBarber = legacyStaffBarber ?? await db.Barbeiros.FirstOrDefaultAsync(b => b.StoreId == primaryStore.Id && b.Nome == staffUsername);
if (staffBarber == null)
{
    staffBarber = new Barbeiro
    {
        StoreId = primaryStore.Id,
        Nome = staffUsername,
        Ativo = true,
    ...
    db.Barbeiros.Add(staffBarber);
    await db.SaveChangesAsync();
}

var configuredStaffPassword = builder.Configuration["DefaultStaff:Password"] ?? builder.Configuration["DEFAULT_STAFF_PASSWORD"];
if (string.IsNullOrWhiteSpace(configuredStaffPassword))
    configuredStaffPassword = builder.Configuration["DefaultKauan:Password"] ?? builder.Configuration["DEFAULT_KAUAN_PASSWORD"];
if (string.IsNullOrWhiteSpace(configuredStaffPassword))
    configuredStaffPassword = defaultStaffPassword;

var staffUser = await db.Users.FirstOrDefaultAsync(u => u.Username == staffUsername && u.StoreId == primaryStore.Id);
if (!string.IsNullOrWhiteSpace(configuredStaffPassword) && staffUser == null)
{
    db.Users.Add(new User
    {
        Username = staffUsername,
        PasswordHash = auth.HashPassword(configuredStaffPassword),
        Role = "barbeiro",
        StoreId = primaryStore.Id,
        BarberId = staffBarber.Id
    });
    await db.SaveChangesAsync();
}
else if (staffUser != null)
{
    if (staffUser.BarberId == null)
    {
        staffUser.BarberId = staffBarber.Id;
        await db.SaveChangesAsync();
    }
}
else
{
    Console.WriteLine($"AVISO: DEFAULT_STAFF_PASSWORD nao definido — usuario '{staffUsername}' nao sera criado.");
}
